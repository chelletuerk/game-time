...
function levelUpRight(difficulty) {
  difficulty.allRight.forEach(function(vehicles) {
    vehicles.forEach(function(vehicle) {
      vehicle.speed = +localStorage.getItem('level');
    })
  })
}

function localCheck(difficulty) {
  var local = localStorage.getItem('level');
  if (!local) localStorage.setItem('level', '1');
  if (local) levelUpRight(difficulty);
}
