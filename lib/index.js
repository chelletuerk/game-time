...
requestAnimationFrame(function gameLoop() {
  ...
  lilypads.all.forEach(function (lilypad){
    lilypad.forEach(function(e) {
      e.draw()
    })
  });
  drowningFrog();
...
function win(lilypads) {
  if (frog.x < lilypads.x + lilypads.width &&
    frog.x + frog.width > lilypads.x &&
    frog.y < lilypads.y + lilypads.height &&
    frog.height + frog.y > lilypads.y){
    level += 3;
    levelUpRight(cars)
    resetFrog();
  }
